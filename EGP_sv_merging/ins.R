...
df = read.table("ins.txt", header=TRUE, sep="\t", as.is=TRUE, na.strings = c("NA"))

dt = as.data.table(df)

res = dt[, list(tsv_id=list(tsv_id), ins_length=list(unique(insertion_length))), by=list(sample, chr, pos)]

res$sv = paste0(res$chr, ":", res$pos)

c(round(mean(table(res$sample))), round(sd(table(res$sample))), round(min(table(res$sample))), round(max(table(res$sample))))


fwrite(res, "ins.collapsed.txt", sep="\t", col.names=TRUE, row.names=FALSE, quote=FALSE)
...
ggsave("ins.collapsed.counts_per_ind_chr.png", plot=last_plot(), dpi=300, width=20, height=10, scale=1.2, units="cm")
...
ggsave("ins.collapsed.counts_per_chr.png", plot=last_plot(), dpi=300, width=20, height=10, scale=1.2, units="cm")
